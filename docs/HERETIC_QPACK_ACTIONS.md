...
TrueNAS `storage-share` HDD share through Tailscale and writes the container
and Hugging Face's resumable upload cache under:

```text
storage-share/Upload/qpack-builds/qwen3.6-35b-heretic.qpack
storage-share/Upload/qpack-builds/.hf-xet-cache
```
...
| `TRUENAS_SMB_USERNAME` | SMB user allowed to write to `storage-share/Upload/qpack-builds` |
...
The GitHub repository already has `TRUENAS_SMB_USERNAME` set to `jbenzaquen`.
Only the password is missing for this account.
...
1. Add the missing repository secrets: `TRUENAS_SMB_PASSWORD`,
   `TS_OAUTH_CLIENT_ID`, `TS_OAUTH_SECRET`, and a fine-grained `HF_TOKEN`.
...
- Use a dedicated SMB account limited to `storage-share/Upload/qpack-builds` if practical.
